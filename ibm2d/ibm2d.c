...
int main(int argc, char **argv) {
    ...
    fscanf(fp_in, "%*s %d", &numvert);
    double vert[numvert][2];
    for (int i = 0; i < numvert; i++) {
        fscanf(fp_in, "%lf %lf", &vert[i][0], &vert[i][1]);
    }
    ...
    for (int i = 0; i < numvert; i++) {
        double a[2] = {
            vert[(i+1)%numvert][0] - vert[i][0],
            vert[(i+1)%numvert][1] - vert[i][1]
        };
        double b[2] = {
            vert[(i+numvert-1)%numvert][0] - vert[i][0],
            vert[(i+numvert-1)%numvert][1] - vert[i][1]
        };
        theta[i] = vecang(a, b);
    }
    ...
    for (int i = 0; i <= Nx+1; i++) {
        for (int j = 0; j <= Ny+1; j++) {
            ...
            for (int k = 0; k < numvert; k++) {
                double s[2] = {vert[(k+1)%numvert][0] - vert[k][0], vert[(k+1)%numvert][1] - vert[k][1]};

                double ap[2] = {p[0] - vert[k][0], p[1] - vert[k][1]};
                double bp[2] = {p[0] - vert[(k+1)%numvert][0], p[1] - vert[(k+1)%numvert][1]};

                double t = -(s[0]*bp[0]+s[1]*bp[1]) / (s[0]*s[0]+s[1]*s[1]);

                double d;
                if (t > 1) {
                    d = dist(vert[k], p);
                    if (vecang(s, ap) < theta[k]) {
                        d = -d;
                    }
                }
                else if (t < 0) {
                    d = dist(vert[(k+1)%numvert], p);
                    if (vecang(bp, (double [2]){-s[0], -s[1]}) < theta[(k+1)%numvert]) {
                        d = -d;
                    }
                }
                else {
                    double h[2] = {
                        t*vert[k][0] + (1-t)*vert[(k+1)%numvert][0],
                        t*vert[k][1] + (1-t)*vert[(k+1)%numvert][1]
                    };
                    d = dist(h, p);
    ...
    for (int i = 1; i <= Nx; i++) {
        for (int j = 1; j <= Ny; j++) {
            if (flag[i][j] == 2) {
                int cur_id = cell_id[i][j];

                double nx = (lvset[i+1][j] - lvset[i-1][j]) / (xc[i+1] - xc[i-1]);
                double ny = (lvset[i][j+1] - lvset[i][j-1]) / (yc[j+1] - yc[j-1]);
                double nl = sqrt(nx*nx + ny*ny);

                nx /= nl;
                ny /= nl;

                double mx = xc[i] - 2*lvset[i][j]*nx;
                double my = yc[j] - 2*lvset[i][j]*ny;

                int iright = bs_upper_bound(Nx+2, xc, mx);
                int jupper = bs_upper_bound(Ny+2, yc, my);
                ...
                double x1 = xc[iright-1], x2 = xc[iright];
                double y1 = yc[jupper-1], y2 = yc[jupper];
